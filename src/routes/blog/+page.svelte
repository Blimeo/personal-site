<script lang="ts">
	...
	const posts = [
	  ...
	  { lat: 40.7128, long: -74.0060, name: 'New York', url: '/blog/nyc' },
...
<div class="min-h-screen">
    ...
    <!-- The Map Container -->
    {#if showMap}
        <div transition:slide={{ duration: 300 }} class="h-56 sm:h-80 w-full mb-6">
    ...
    {/if}
                <ul class="list-disc list-inside space-y-1">
                    {#each data.posts as post}
                        <li>
                            <a class="text-sky-400 hover:underline" href={'/blog/' + post.slug}>{post.title}</a>
							- {post.description}
